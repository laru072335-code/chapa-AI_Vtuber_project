...
class unityserver:
    ...
    """
    def __init__(self,responce_queue=None):
        self.connected = set()
    ...
    async def send_viseme(self,in_q):
        """
        visemeの送信
        """
        while True:
    ...
    async def send_subtitle(self,prompt_in_q,answer_in_q):
        """
        字幕の送信
        """
        while True:
            ...
            subtitle=[subtitle_prompt,subtitle_answer]
            await self._send_data(1,subtitle)

    async def send_au(self,in_q):
        """
        auの送信
        """
        while True:
    ...
    async def send_sound(self,in_q):
        """
        音の送信
        """
        while True:
    ...
    async def _send_data(self,head,data):
        """
        ヘッダをつけて送る用
        """
