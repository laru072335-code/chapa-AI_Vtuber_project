...
def test_meaning_vector_and_emotion_vector(sentence):
    """
    興味推定や、意味（興味）ベクトルの生成ができているかを確認する。
    """
    au_list,interest,emotion_probs=analyze.analyze(sentence,"test",test=True)
    assert interest.shape==(768,) #ここ本当は(1,768)にしたい
...
async def test_generate_voice(sentence):
    ...
    voice=Voice()
    work=asyncio.create_task(voice.voicevox_generate_voice(
        Client,
        46,
        prompt_queue,
        sound_queue,
